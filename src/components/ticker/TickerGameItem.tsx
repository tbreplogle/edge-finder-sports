...
export const TickerGameItem = ({ game }: Props) => {
  ...
  /* ---------------- RENDER ---------------- */
  return (
    <div className="flex items-center justify-between px-5 py-3 bg-edge-primary text-white rounded-none whitespace-nowrap border-l border-r border-edge-neutral/30">
      {isFinal ? (
        <>
          {/* Final game display */}
          <div className="flex items-center">
            <div className="flex flex-col mr-4">
              ...
              <span className="font-medium text-sm mt-1.5">{homeTeam}</span>
            </div>
            <div className="flex flex-col">
              <span className="font-medium text-sm">{game.score_away}</span>
              <span className="font-medium text-sm mt-1.5">{game.score_home}</span>
            </div>
          </div>
          <Badge variant="outline" className="text-[10px] px-1.5 py-0.5 text-secondary border-secondary ml-3">
            FINAL
          </Badge>
        </>
      ) : (
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex items-center ml-4">
            ...
            {game.tip && (
              <span className="text-xs font-semibold ml-4">{game.tip}</span>
            )}
          </div>
        </div>
      )}
