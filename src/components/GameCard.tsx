...
export function GameCard(p: GameCardProps) {
  ...
  const fmtMl = (v: number | null) => (v == null ? "—" : v > 0 ? `+${v}` : `${v}`);
      ...
      <Card className="overflow-hidden border-0 bg-card shadow-lg relative">
        <AspectRatio ratio={16 / 7} className="bg-gradient-to-r from-edge-primary to-[#1f3356] text-white">
          ...
          <div className="absolute top-4 right-4">
            <Badge variant="outline" className="flex items-center gap-1 bg-black/30 border-white/20">
          ...
          <div className="pt-12 px-6 pb-6 flex flex-col h-full space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {(["away", "home"] as const).map((side) => {
                ...
                const teamId  = isAway ? p.away_team_id   : p.home_team_id;
                ...
                const logoSrc = TEAM_LOGOS[teamId] ?? "";
  ...
  return (
    <Card className="edge-card">
      <CardContent className="p-4 space-y-2">
        ...
        {/* header row */}
        <div className="grid grid-cols-5 text-xs text-muted-foreground mb-1">
          <div /> {/* team label + logo */}
          <div className="text-center col-span-1">Market</div>
          <div className="text-center col-span-1">Predicted</div>
          <div className="text-center col-span-1">Edge</div>
        </div>
        ...
        {(["away", "home"] as const).map((side) => {
          ...
          const tmId    = isAway ? p.away_team_id   : p.home_team_id;
          ...
          const prPct   = isAway ? p.away_pred_pct  : p.home_pred_pct;
          const edge    = isAway ? p.away_edge_pct  : p.home_edge_pct;
          const logoSrc = TEAM_LOGOS[tmId] ?? "";

          return (
            <div key={side} className="grid grid-cols-5 items-center mb-1 gap-x-1">
              {/* team & pitcher */}
              ...
              <div
                className={cn(
                  "text-center col-span-1 flex items-center justify-center gap-1",
